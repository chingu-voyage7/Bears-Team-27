...
class Upload extends React.Component {
  ...
  uploadFile = (file, progressCallback) => {
    if (file) {
      ...
      xhr.onload = async () => {
        ...
        const resource = {name: 'check1.jpg', secureUrl: response.secure_url};

        await this.saveResourceRefToMongo(resource);
  ...
  saveResourceRefToMongo = async resource => {
    ...
    data.append('json', JSON.stringify(resource));
    //console.dir(data);
    console.log('resource');
    console.log(resource);
    const res = await fetch('http://localhost:3000/api/resources/new', {
      method: 'POST',
      body: {name: 'rachna.jpg', secureUrl: 'http://testurl.com'},
    });
  };
  filesToItems(files) {
