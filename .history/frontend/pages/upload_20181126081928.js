...
class Upload extends React.Component {
  ...
  clearIfAllCompleted() {
    ...
    if (completed === this.state.items.length) {
      setTimeout(() => {
        this.setState({items: [], message: 'All Files Uploaded Successfully!'});
  ...
  uploadFile = (file, progressCallback) => {
    if (file) {
      ...
      xhr.onload = () => {
        ...
        const resource = {name: file.name, secureUrl: response.secure_url};
  ...
  render() {
    return (
      <div >
        ...
        {this.state.message.length > 0 && <label>{this.state.message}</label>}
